...
# Check whether or not the call rate of the samples exceed a certain level
# This does not necessarily has to be different between the PGx pipeline and GAP.

# We probably want more stringent filters here:
# Which variants must absolutely be present prior to imputation?
# Perhaps we want to work per pharmacogene (so we can return results for what is available)
for chr in {1..22} "XY"
do
...
for chr in {1..22} "XY"
 ...
 # Now remove variants that we know we can genotype given validation in
 # GDIO samples.
