...
bedtools flank -b 1000000 -i ~/pgx-pipeline/validation/pgx-genes_GRCh37.bed -g hg19.genome > \
~/pgx-pipeline/validation/pgx-genes_GRCh37_1Mb-flanks.bed

mysql --user=genome --host=genome-mysql.cse.ucsc.edu -A -e "select chrom, size from hg38.chromInfo" | \
sed '2,$s/chr//g' > hg38.genome

bedtools flank -b 1000000 -i ~/pgx-pipeline/validation/pgx-genes_GRCh38.bed -g hg38.genome > \
~/pgx-pipeline/validation/pgx-genes_GRCh38_1Mb-flanks.bed